...
std::tuple<int, int> finding1PTM ( float masses [],float prob [], const int &PTM_list, const float &mass_shift_lower, const float &mass_shift_upper) {
		...
		return {highest_prob_pos_1, pos_length};
}
...
std::tuple<vector<int>, int, vector<int>, int> findingCombinationOf2and3PTM ( float masses [],float prob [], const int &PTM_list, const float &mass_shift_lower, const float &mass_shift_upper) {
	
	//vector<int> position_combination2; // here is were we store all matched positions
	int list_end = PTM_list-1;

	vector<int> highest_prob_pos_2 {PTM_list, PTM_list};
	int combination_2 = 0;
	vector<int> highest_prob_pos_3 {PTM_list, PTM_list, PTM_list};
	int combination_3 = 0;
	...
	float maximum = mass_shift_upper - (-(list_lowest_value)*2);//the maximum mass shift we need to check in the first loop 
	int limit {};
...
//				int limit_2 = binarySearchLargerElement( masses, maximum_2, PTM_list);
		...
		return {highest_prob_pos_2, combination_2, highest_prob_pos_3, combination_3};
}
					...
					while (nat_prob[y] + nat_prob[x] + nat_prob[i] + nat_prob[j] > sum_of_probs_4 && j < nat_prob_length) {
                        ...
                        if ((sum <= mass_shift_upper) && (sum >= mass_shift_lower)) {
							...
							if (prob_sum >= nat_prob[highest_prob_pos_4[0]] + nat_prob[highest_prob_pos_4[1]] + nat_prob[highest_prob_pos_4[2]] + nat_prob[highest_prob_pos_4[3]]) {
								...
								sum_of_probs_5 = prob_average_4*5;
							}
							combination_4++;
						}
...
std::tuple<vector<int>, int, vector<int>, int> findingCombinationOf4and5PTM_Best (float masses [],float prob [], const int &PTM_list, const float &mass_shift_lower, const float &mass_shift_upper) {
	...
	int list_end = PTM_list -1;
	vector<int> highest_prob_pos_4 {PTM_list, PTM_list, PTM_list, PTM_list};
	int combination_4 = 0;
	vector<int> highest_prob_pos_5 {PTM_list, PTM_list, PTM_list, PTM_list, PTM_list};
	int combination_5 = 0;
...
float* openFile(string path, int length) {
	...
	int count = 0;
	while (!inputFile.eof()) {
		inputFile >> array_ptr[count];
		count++;
       //cout << array_ptr[count-1]<<endl;
	}
...
//	cout << masses[0] << " ";
//	cout << masses[PTM_list-1] << endl;
	
	//for (int x {0}; x <= PTM_list; x++)
		//cout << masses[x] << endl;

	// Probabilities array
	float* prob = openFile(dir_path + libs+  "new_prob.txt", 1499);
	//cout << endl << "Prob"   << endl;
	//cout << prob[0] << " " << prob[PTM_list-1] << " " << prob[PTM_list] << endl;
	// null probability for no matches (last new vector position)
	prob[PTM_list] = 0.0;
	//cout << prob[0] << " " << prob[PTM_list-1] <<  " " << prob[PTM_list] << endl;
	//for ( int x {0}; x <= list_end; x++)
		//cout << prob[x] << endl;
	...
	//cout << endl << "NAT_Prob"   << endl;
	//cout << nat_prob[0] << " " << nat_prob[nat_list-1] << " " << nat_prob[nat_list] << endl;
	nat_prob[nat_list] = 0.0;
	//cout << nat_prob[0] << " " << nat_prob[nat_list-1] << " " << nat_prob[nat_list] << endl;
	// Nat masses array
	float* nat_masses = openFile(dir_path + libs+  "new_nat_masses.txt", nat_list);
	//cout << endl << "NAT_Masses"   << endl;
	//cout << nat_masses[0] << " " << nat_masses[nat_list-1] << " " << nat_masses[nat_list] << endl;
...
//        float peptide_mass = 1000; 
		...
		auto [highest_prob_pos_1, pos_length] = finding1PTM(masses, prob, PTM_list, mass_shift_lower, mass_shift_upper);
		...
		if (combination_1 > 0) {
			prob_average_1 = prob[highest_prob_pos_1];
			//cout << "\n" <<  "The mass shift can be explained by 1 PTM  - ";
			//cout << "with score: " << prob_average_1 << "\n";
			highest_prob_pos[0] = highest_prob_pos_1;
		...
		auto [highest_prob_pos_2, combination_2, highest_prob_pos_3, combination_3] = findingCombinationOf2and3PTM(masses, prob, PTM_list, mass_shift_lower, mass_shift_upper);
...
//    cout << prob_average_3 << " " <<prob_average_4 << "\n";
		...
		if (more_than_3 <= 3) {
			...
			inputPTMnames.open(dir_path + libs + "new_names.txt");
			//string sline;
...
//			}
		...
		//// 4-5 PTMs
			...
			Natural_PTM_Names.open(dir_path + libs+  "new_nat_names.txt");
			//cout << "\nPTM combination: ";
