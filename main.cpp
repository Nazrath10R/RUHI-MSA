...
int main( ) {
	...
	string sample = "JOHNPP_citr_all";

	string dir_path = "/data/SBCS-BessantLab/naz/ptm/";
...
//        float peptide_mass = 1000;
		...
		vector<int> highest_prob_pos = {list_end, list_end, list_end, list_end, list_end};
		...
		for (int x = 0; x < nat_prob_length; x++) {
			...
			for (int i = x; i < nat_prob_length; i++) {
				...
				for (int j = i; j < nat_prob_length; j++) {
					...
					while (nat_prob[x] + nat_prob[i] + nat_prob[j] + nat_prob[k] > prob_vector_max_1 && k <= nat_list) {
			...
			for (int x = y; x < nat_prob_length; x++) {
				...
				for (int i = x; i < nat_prob_length; i++) {
					...
					for (int j = i; j < nat_prob_length; j++) {
						...
						while (nat_prob[y] + nat_prob[x] + nat_prob[i] + nat_prob[j] + nat_prob[k] > prob_vector_max_2 && k <= nat_list) {
